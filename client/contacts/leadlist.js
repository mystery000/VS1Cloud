        ...
        setTimeout(function () {
            $('#tblLeadlist').DataTable({
                ...
                "fnInitComplete": function () {
                    $("<button class='btn btn-primary btnRefreshLeads' type='button' id='btnRefreshLeads' style='padding: 4px 10px; font-size: 14px; margin-left: 12px !important;'><i class='fas fa-search-plus' style='margin-right: 5px'></i>Search</button>").insertAfter("#tblLeadlist_filter");
                }
...
Template.leadlist.events({
        ...
        if (dataSearchName.replace(/\s/g, '') !== '') {
            sideBarService.getLeadByNameOrID(dataSearchName).then(function (data) {
                ...
                if (data.tprospect.length > 0) {
                    ...
                    $('.fullScreenSpin').css('display', 'none');
                    if (dataTableList) {
                        const datatable = $('#tblLeadlist').DataTable();
    ...
    'click .resetTable' : function(event){
        let checkPrefDetails = getCheckPrefDetails(); //tblEmployeelist
        if (checkPrefDetails) {
    ...
    'blur .divcolumn' : function(event){
        ...
        const datable = $('#tblLeadlist').DataTable();
