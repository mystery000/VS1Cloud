...
import "./mail-chimp.html";

const settingService = new TaxRateService();
const settingFields = [
  "VS1MailchimpApiKey",
  "VS1MailchimpAudienceID",
  "VS1MailchimpCampaignID",
];
...
Template.mailchimp.onRendered(function () {
  ...
  templateObject.getSettingsList = async function () {
    ...
    if (details.length > 0) {
      ...
      $("#openMailchimpLink").html(
        `<i class="fa fa-phone" style="margin-right: 8px;"></i>Log into Mail Chimp`
      );
    } else {
      $("#openMailchimpLink").html(
        `<i class="fa fa-phone" style="margin-right: 8px;"></i>Mail Chimp Sign Up`
      );
    }
