                        ...
                        $.grep(templateObject.formsData.get(), async(n) => {
                            ...
                            for (let i = 0; i < empData.length; i++) {
                                if ((n.id == '1' && empData[i].fields.BeginFromOption === "S") || (n.id == empData[i].fields.FormID && empData[i].fields.BeginFromOption !== "S")) {
                                    ...
                                    if (basedOnTypeText != '' && basedOnType.split(',').includes('F')) basedOnTypeText = ', ' + basedOnTypeText.slice(0, -2);
                                    else if(basedOnTypeText != '') basedOnTypeText = basedOnTypeText.slice(0, -2);
                                    empDataCurr = {
            ...
            try {
                let promise = settings.map(async (setting) => {
                    ...
                    async function getScheduleID (employeeId) {
                        return new Promise(async(resolve, reject)=> {
                            getVS1Data('TReportSchedules').then(dataObject => {
                                if(dataObject.length == 0) {
                                ...
                                }).catch(function(error) {resolve(-1)})
                            })
                        })
                    }
                    if (!!recipients) {
                                        ...
                                        await (async () => {
                                            return new Promise((resolve, reject) => {
                                                    html2pdf().set(opt).from(source).toPdf().output('datauristring').then((dataObject) => {
                                                        ...
                                                        let tDate = '';
                                                        let dDate = '';
                                                        if (transIDs.includes(formID.toString()) == true && formID != 177 && formID != 61 && formID !=94) {
                                                            tDate = source.querySelector('#sale-date').innerHTML;
                                                            dDate = source.querySelector('#due-date').innerHTML;
                                                        }
            ...
            if (!!recipients) {
                ...
                let savePromise = recipientIds.map(async (recipientId, index) => {
                    ...
                    let promises = groupedReports.map(async (groupedReport) => {
                        ...
                        await taxRateService.saveScheduleSettings(objDetail).then(dataReturn=>{
                            taxRateService.getScheduleSettings().then(dataUpdate => {
                                addVS1Data('TReportSchedules', JSON.stringify(dataUpdate)).then(function(){
                                })
                            })
                        }).catch(function(err) {
                        });
...
Template.emailsettings.events({
    ...
    'click #emailsetting-normal': async function () {
        const templateObject = Template.instance();
        const normalSettings = $('#tblAutomatedEmails tbody tr.dnd-moved').map(function () { return $(this) }).get();
        $('.fullScreenSpin').css('display', 'inline-block');
