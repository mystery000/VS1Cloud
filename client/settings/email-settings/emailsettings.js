...
Template.emailsettings.onCreated(function () {
    ...
    let headerStructure1  = [
        ...
        { index: 5, label: 'Memo', class: 'colTemplateContent', active: true, display: true, width: "" },
        { index: 6, label: '', class: 'colDelete', active: true, display: true, width: "60" }
    ];
                ...
                if (dataObject.length > 0) {
                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            $.grep(templateObject.formsData.get(), async (n) => {
                                ...
                                for (let i = 0; i < empData.length; i++) {
                                    if (
                                        (n.id == "1" &&
                                            empData[i].fields.BeginFromOption === "S") ||
                                        (n.id == empData[i].fields.FormID &&
                                            empData[i].fields.BeginFromOption !== "S")
                                    ) {
                                        ...
                                        if (basedOnType.split(",").includes("S"))
                                            basedOnTypeText += "On Save, ";
                                        if (basedOnType.split(",").includes("T"))
                                            basedOnTypeText += "On Transaction Date, ";
                                        if (basedOnType.split(",").includes("D"))
                                        ...
                                        if (
                                            ...
                                            basedOnTypeText = ", " + basedOnTypeText.slice(0, -2);
                                        else if (basedOnTypeText != "")
                                            basedOnTypeText = basedOnTypeText.slice(0, -2);
                                        empDataCurr = {
                                        ...
                                        if (recipients.length === 1 && formIds.length === 1)
                                            employeeScheduledRecord.push(empDataCurr);
                                        else {
                                            employeeScheduledRecord = [
                                                ...employeeScheduledRecord.filter(
                                                    (schedule) => schedule.formID != n.id
                                                ),
                                                empDataCurr,
                                            ];
                                        }
                                ...
                                let found = employeeScheduledRecord.some(
                                    (checkdata) => checkdata.formID == n.id
                                );
                                if (!found) {
                                    employeeScheduledRecord.push(empDataCurr);
                                }
                    ...
                    taxRateService
                        ...
                        .then(async function (data) {
                                    ...
                                    $.grep(templateObject.formsData.get(), async (n) => {
                                        ...
                                        for (let i = 0; i < empData.length; i++) {
                                            if (
                                                ...
                                                if (
                                                    !recipients.includes(empData[i].fields.Recipients)
                                                ) {
                                                    recipients.push(empData[i].fields.Recipients);
                                                    recipientIds.push(empData[i].fields.EmployeeID);
                                                }
                                                if (
                                                    ...
                                                    formIds.push(empData[i].fields.FormID);
                                                const startDate = empData[i].fields.StartDate.split(
                                                    " "
                                                )[0];
                                                const startTime = empData[i].fields.StartDate.split(
                                                    " "
                                                )[1];
                                                ...
                                                empDataCurr = {
                                                    ...
                                                    sunAction: empData[i].fields.SunAction || "",
                                                    beginFromOption:
                                                        empData[i].fields.BeginFromOption || "",
                                                    formIDs: formIds.join("; "),
                                                };
                                                if (recipients.length === 1 && formIds.length === 1)
                                                    employeeScheduledRecord.push(empDataCurr);
                            ...
                            $(".fullScreenSpin").css("display", "none");
                            templateObject.employeescheduledrecord.set(
                                employeeScheduledRecord
                            );
