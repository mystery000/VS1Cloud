    ...
    'click .resetTable' : function(event){
      ...
      if(getcurrentCloudDetails){
        if (getcurrentCloudDetails._id.length > 0) {
          ...
          var checkPrefDetails = CloudPreference.findOne({userid:clientID,PrefName:'termsList'});
          if (checkPrefDetails) {
        ...
        if (getcurrentCloudDetails._id.length > 0) {
          ...
          if (checkPrefDetails) {
            CloudPreference.update({_id: checkPrefDetails._id},{$set: { userid: clientID,username:clientUsername,useremail:clientEmail,
              PrefGroup:'salesform',PrefName:'termsList',published:true,
...
Template.termsettings.helpers({
  ...
  return CloudPreference.findOne({userid:Session.get('mycloudLogonID'),PrefName:'termsList'});
},
deptrecords: () => {
