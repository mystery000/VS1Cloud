...
Template.edismssettings.onCreated(() => {
  const templateObject = Template.instance();
  templateObject.smsSettingDetails = new ReactiveVar([]);
  templateObject.saveSettingObject = async function (data) {
    return new Promise((resolve) => {
      const oldSetting = templateObject.smsSettingDetails
        .get()
        .filter((field) => field.PrefName === data.eKey);
      let settingObject = {};
...
Template.edismssettings.onRendered(function () {
...
Template.edismssettings.events({
  ...
  "click #saveTwilioSettings": async function () {
    ...
    setTimeout(async function () {
      ...
      for (const eKey of allKeys) {
        ...
        switch (eKey) {
          case "VS1SMSID":
            value = $("#twilioAccountId").val();
            tag = "sid,twilliotoken,phone";
            break;
          case "VS1SMSToken":
            value = $("#twilioAccountToken").val();
            tag = "sid,twilliotoken,phone";
            break;
          case "VS1SMSPhone":
            value = $("#twilioTelephoneNumber").val();
            tag = "sid,twilliotoken,phone";
            break;
          case "VS1HEADERSMSMSG":
            value = $("#headerAppointmentSMS").val();
            break;
