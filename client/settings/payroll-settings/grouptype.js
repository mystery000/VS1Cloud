...
Template.grouptypelistpop.onCreated(() => {
    const templateObject = Template.instance();
    templateObject.grouptypelist = new ReactiveVar([]);
});

Template.grouptypelistpop.onRendered(function() {
        ...
        getVS1Data('TPayrollHolidayGroup').then(function(dataObject) {
            if (dataObject.length == 0) {
                sideBarService.getNewGroupListVS1().then(function(data) {
                    let records = [];
                    let inventoryData = [];
                    addVS1Data('TPayrollHolidayGroup',JSON.stringify(data));
                    for (let i = 0; i < data.tpayrollholidaygroup.length; i++) {
                       if (!isNaN(data.tpayrollholidaygroup[i].fields.Groupdesc)) {
                        description = data.tpayrollholidaygroup[i].fields.Groupdesc || '';
                      } else {
                        description = "";
                      }
                      var dataList = [                   
                    ...
                    if(splashArrayGroupTypeList) {
                        $('#tblgrouptypelist').dataTable({
                            ...
                            "fnInitComplete": function () {
                              $("<button class='btn btn-primary btnAddGroupType' data-dismiss='modal' data-toggle='modal' data-target='#newGroupModal' type='button' style='padding: 4px 10px; font-size: 14px; margin-left: 12px !important;'><i class='fas fa-plus'></i></button>").insertAfter("#tblgrouptypelist_filter");
                              $("<button class='btn btn-primary btnRefreshGroupType' type='button' id='btnRefreshGroupType' style='padding: 4px 10px; font-size: 14px; margin-left: 12px !important;'><i class='fas fa-search-plus' style='margin-right: 5px'></i>Search</button>").insertAfter("#tblgrouptypelist_filter");
                            }
                ...
                for (let i = 0; i < useData.length; i++) {
                    if (!isNaN(useData[i].fields.Groupdesc)) {
                        description = useData[i].fields.Groupdesc || '';
                    } else {
                        description = '';
                    }
                    var dataList = [
...
Template.grouptypelistpop.events({
    ...
    'click .btnRefreshGroupType': function (event) {
        ...
        let sideBarService = new SideBarService();
        let rateTypeService = new RateTypeService();
        let dataSearchName = $('#tblgrouptypelist_filter input').val();
        var currentLoc = FlowRouter.current().route.path;
        if (dataSearchName.replace(/\s/g, '') != '') {
            sideBarService.getOneGroupTypeByName(dataSearchName).then(function (data) {
                let lineItems = [];
                let lineItemObj = {};
                if (data.tpayrollholidaygroup.length > 0) {
                  for (let i = 0; i < data.tpayrollholidaygroup.length; i++) {
                    ...
                    swal({
                        title: 'Question',
                        text: "Group does not exist, would you like to create it?",
                        type: 'question',
    ...
    'click .lineDescription': function(event) {
        ...
        $('#selectGroupLineID').val(targetID);
