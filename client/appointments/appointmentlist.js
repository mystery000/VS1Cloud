...
Template.appointmentlist.onRendered(async function () {
        ...
        .then(function (dataObject) {
          if (dataObject.length == 0) {
            ...
            if (result.terppreference.length > 0) {
              for (let i = 0; i < result.terppreference.length; i++) {
                if (
                  result.terppreference[i].PrefName == "VS1SMSID" ||
                  result.terppreference[i].PrefName == "VS1SMSToken" ||
                  result.terppreference[i].PrefName == "VS1SMSPhone" ||
                  result.terppreference[i].PrefName == "VS1SAVESMSMSG" ||
                  result.terppreference[i].PrefName == "VS1STARTSMSMSG" ||
                  result.terppreference[i].PrefName == "VS1STOPSMSMSG"
                ) {
                  switch (result.terppreference[i].PrefName) {
                    case "VS1SMSID":
                      smsSettings.twilioAccountId =
  ...
  templateObject.getAllAppointmentListData = async function () {
    ...
    getVS1Data("TAppointmentList")
      .then(async function (dataObject) {
        if (dataObject.length == 0) {
          sideBarService
            .getTAppointmentListData(
              prevMonth11Date,
              toDate,
              true,
              initialReportLoad,
            ...
            .then(function (data) {
              ...
              for (let i = 0; i < data.tappointmentlist.length; i++) {
                ...
                if (data.tappointmentlist[i].Active == true) {
                  if (
                    data.tappointmentlist[i].Status == "Converted" ||
                    data.tappointmentlist[i].Status == "Completed"
                  ) {
                    color = "#1cc88a";
                  } else {
                    color = "#f6c23e";
                ...
                var dataList = {
                  ...
                  sortdate:
                    data.tappointmentlist[i].CreationDate != ""
                      ? moment(data.tappointmentlist[i].CreationDate).format(
                          "YYYY/MM/DD"
                        )
                      : data.tappointmentlist[i].CreationDate,
                  appointmentdate:
                  ...
                  statementno:
                    data.tappointmentlist[i].EnteredByEmployeeName || "",
                  employeename:
                    data.tappointmentlist[i].EnteredByEmployeeName || "",
                  ...
                  startTime:
                    data.tappointmentlist[i].STARTTIME.split(" ")[1] || "",
                  timeStart: moment(data.tappointmentlist[i].STARTTIME).format(
                    "h:mm a"
                  ),
                  timeEnd: moment(data.tappointmentlist[i].ENDTIME).format(
                    "h:mm a"
                  ),
                  ...
                  frmDate:
                    moment(data.tappointmentlist[i].STARTTIME).format("dddd") +
                    ", " +
                    moment(data.tappointmentlist[i].STARTTIME).format("DD"),
                  toDate:
                    moment(data.tappointmentlist[i].ENDTIME).format("dddd") +
                    ", " +
                    moment(data.tappointmentlist[i].ENDTIME).format("DD"),
                  fromDate:
                    data.tappointmentlist[i].Actual_Endtime != ""
                      ? moment(data.tappointmentlist[i].Actual_Endtime).format(
                          "DD/MM/YYYY"
                        )
                      : data.tappointmentlist[i].Actual_Endtime,
                  openbalance: data.tappointmentlist[i].Actual_Endtime || "",
                  aStartTime:
                    data.tappointmentlist[i].Actual_Starttime.split(" ")[1] ||
                    "",
                  aEndTime:
                    data.tappointmentlist[i].Actual_Endtime.split(" ")[1] || "",
                  actualHours: "",
                  closebalance: "",
                  ...
                  color: color,
                  actual_starttime:
                    data.tappointmentlist[i].Actual_Starttime || "",
                ...
                if (accessLevel) {
                  if (
                    data.tappointmentlist[i].CUSTFLD13 === "Yes" &&
                    data.tappointmentlist[i].CUSTFLD11 === "" &&
                    data.tappointmentlist[i].Active == true
                  ) {
                    ...
                    if (
                      ...
                      const sentSMSs =
                        recentSMSLogs.sms_messages.filter(
                          (message) =>
                            message.from ===
                              "+" +
                                smsSettings.twilioTelephoneNumber.replace(
                                  "+",
                                  ""
                                ) &&
                            message.to ===
                              "+" +
                                data.tappointmentlist[i].Mobile.replace("+", "")
                        ) || "";
                      const receiveSMSs =
                        recentSMSLogs.sms_messages.filter(
                          (message) =>
                      ...
                      if (sentSMSs.length > 0) {
                        for (let j = 0; j < sentSMSs.length; j++) {
                          if (
                            data.tappointmentlist[i].CUSTFLD12 ===
                            sentSMSs[j].sid
                          ) {
                            currentSentSMSDate = sentSMSs[j].date_sent;
                            nextSentSMSDate =
                              j - 1 >= 0 ? sentSMSs[j - 1].date_sent : null;
                            break;
                          }
                        }
                        if (currentSentSMSDate) {
                          for (let j = 0; j < receiveSMSs.length; j++) {
                            const receiveSMSDate = moment(
                              receiveSMSs[j].date_sent
                            );
                            if (
                              receiveSMSDate >= moment(currentSentSMSDate) &&
                              (!nextSentSMSDate ||
                                (nextSentSMSDate &&
                                  receiveSMSDate <= moment(nextSentSMSDate)))
                            ) {
                              const replyText = receiveSMSs[j].body
                                ? receiveSMSs[j].body.toLowerCase()
                                : "";
              ...
              let confirmedColumn =
                '<i class="fas fa-minus-circle text-info" style="font-size: 35px;" data-toggle="tooltip" data-placement="top" title="No SMS Message Sent"></i>';
              for (let p = 0; p < dataTableList.length; p++) {
                if (dataTableList[p].custFld13 == "Yes") {
                  if (dataTableList[p].custFld11 == "Yes") {
                    confirmedColumn =
                      '<i class="fa fa-check text-success" style="font-size: 35px;" data-toggle="tooltip" data-placement="top" title="SMS Message confirmed"></i>';
                  } else if (dataTableList[p].custFld11 == "No") {
                    confirmedColumn =
                      '<i class="fa fa-close text-danger" style="font-size: 35px;" data-toggle="tooltip" data-placement="top" title="SMS Message declined"></i>';
                  } else {
                    confirmedColumn =
                      '<i class="fa fa-question text-warning" style="font-size: 35px;" data-toggle="tooltip" data-placement="top" title="SMS Message no reply"></i>';
                  }
                } else {
                  confirmedColumn =
                    '<i class="fas fa-minus-circle text-info" style="font-size: 35px;" data-toggle="tooltip" data-placement="top" title="No SMS Message Sent"></i>';
                ...
                var dataListAppointmentList = [
                  '<div class="custom-control custom-checkbox pointer" style="width:15px;"><input class="custom-control-input chkBox notevent pointer" type="checkbox" id="f-' +
                    dataTableList[p].id +
                    '" name="' +
                    dataTableList[p].id +
                  ...
                  '<span style="display:none;">' +
                    dataTableList[p].sortdate +
                    "</span> " +
                    dataTableList[p].appointmentdate || "",
                ...
                $("#tblappointmentlist")
                  .DataTable({
                    ...
                    columnDefs: [
                      ...
                      },
                      {
                        ...
                        createdCell: function (
                          ...
                          if (
                            rowData[10] == "Converted" ||
                            rowData[10] == "Completed"
                          ) {
                    ...
                    fnDrawCallback: function (oSettings) {
                      ...
                      $(
                                ...
                                .then(function (dataObjectold) {
                                  if (dataObjectold.length == 0) {
                                    ...
                                    var thirdaryData = $.merge(
                                      $.merge(
                                        [],
                                        dataObjectnew.tappointmentlist
                                      ),
                                      dataOld.tappointmentlist
                                    );
                                    let objCombineData = {
                        ...
                        sideBarService
                          ...
                          .then(function (dataObjectnewApp) {
                            getVS1Data("TAppointment")
                              .then(function (dataObjectoldApp) {
                                if (dataObjectoldApp.length == 0) {
                                } else {
                                  let dataOldApp = JSON.parse(
                                    dataObjectoldApp[0].data
                                  );

                                  var thirdaryDataApp = $.merge(
                                    $.merge(
                                      [],
                                      dataObjectnewApp.tappointmentex
                                    ),
                                    dataOldApp.tappointmentex
                                  );
                                  ...
                                  addVS1Data(
                                    "TAppointment",
                                    JSON.stringify(objCombineDataApp)
                                  )
                                    .then(function (datareturnApp) {
                    ...
                    fnInitComplete: function () {
                      ...
                      //}
                      $(
                        "<button class='btn btn-primary btnRefreshAppointment' type='button' id='btnRefreshAppointment' style='padding: 4px 10px; font-size: 14px; margin-left: 8px !important;'><i class='fas fa-search-plus' style='margin-right: 5px'></i>Search</button>"
                      ).insertAfter("#tblappointmentlist_filter");
              ...
              if (
                ...
                if (
                  ...
                  if (sentSMSs.length > 0) {
                    for (let j = 0; j < sentSMSs.length; j++) {
                      if (
                        data.tappointmentlist[i].CUSTFLD12 === sentSMSs[j].sid
                      ) {
                        currentSentSMSDate = sentSMSs[j].date_sent;
                        nextSentSMSDate =
                          j - 1 >= 0 ? sentSMSs[j - 1].date_sent : null;
                          ...
                          getVS1Data("TAppointmentList")
                            .then(function (dataObjectold) {
                              if (dataObjectold.length == 0) {
                                ...
                                var thirdaryData = $.merge(
                                  $.merge([], dataObjectnew.tappointmentlist),
                                  dataOld.tappointmentlist
                                );
                                let objCombineData = {
                                  Params: dataOld.Params,
                        ...
                        getVS1Data("TAppointment")
                          .then(function (dataObjectoldApp) {
                            if (dataObjectoldApp.length == 0) {
                              ...
                              var thirdaryDataApp = $.merge(
                                $.merge([], dataObjectnewApp.tappointmentex),
                                dataOldApp.tappointmentex
                              );
                              let objCombineDataApp = {
                                tappointmentex: thirdaryDataApp,
...
>>>>>>> Stashed changes
  ...
  templateObject.getAllFilterAppointmentListData = function (
    fromDate,
    toDate,
    ignoreDate
  ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      $.ajax({
        ...
        url:
          ...
          smsSettings.twilioAccountId +
          `/SMS/Messages.json?PageSize=1000`,
        dataType: "json",
...
Template.appointmentlist.events({
  ...
  "change #hideConverted": function () {
    ...
    if (checkbox.checked) {
      if (useData.length > 0) {
        ...
        for (let i = 0; i < useData.length; i++) {
          if (
            useData[i].finished != "Converted" ||
            useData[i].finished == "Completed"
          ) {
            var dataList = {
              id: useData[i].id || "",
  ...
  "click .btnRefresh": function () {
    ...
    sideBarService
      .getGlobalSettings()
      .then(function (dataPrefrences) {
        addVS1Data("TERPPreference", JSON.stringify(dataPrefrences))
          .then(function (datareturn) {})
          .catch(function (err) {});
      })
      .catch(function (err) {});
  ...
  "change #dateTo": function () {
    ...
    setTimeout(function () {
      ...
      if (
        ...
        templateObject.getAllFilterAppointmentListData(
          formatDateFrom,
          formatDateTo,
          false
        );
...
>>>>>>> Stashed changes
  ...
  "submit #frmAppointment": function (event) {
    ...
    let startDate =
      ...
      "-" +
      (startdateGet.getMonth() + 1) +
      "-" +
      startdateGet.getDate();
    let endDate =
      endDateGet.getFullYear() +
      "-" +
      (endDateGet.getMonth() + 1) +
      "-" +
      endDateGet.getDate();
    ...
    let selectedProduct =
      $("#product-list").children("option:selected").text() || "";
  ...
  "click #btnEndActualTime": function () {
    if (document.getElementById("tActualStartTime").value == "") {
      ...
      document.getElementById("tActualEndTime").value =
        ...
        ":" +
        moment().startOf("minute").format("mm");
      var actualStartTime = moment(
        document.getElementById("tActualStartTime").value,
        "HH:mm"
      ).format("HH:mm");
      var actualEndTime = moment(
        document.getElementById("tActualEndTime").value,
        "HH:mm"
      ).format("HH:mm");
  ...
  "click .chkBox": async function () {
    ...
    let appointmentService = new AppointmentService();
    var selectedClient = $(this.target)
      .closest("tr")
      .find(".colAccountName")
      .text();
    const templateObject = Template.instance();
    ...
    $(".chkBox:checkbox:checked").each(async function () {
      ...
      if (selectAppointment.length > 0) {
        let checkAppointStartEnd = selectAppointment.filter(
          (item) => parseInt(item.id) == parseInt(chkIdLine)
        );
        let actuatlStartTime =
          checkAppointStartEnd[0].actual_starttime.split(" ")[0] || "";
        ...
        if (oPost.readyState == 4 && oPost.status == 200) {
          ...
          if (myArrResponse.ProcessLog.ResponseStatus.includes("OK")) {
            ...
            appointmentService
              ...
              .then(function (data) {
                FlowRouter.go(
                  "/invoicelist?success=true&apptId=" +
                    parseInt(selectAppointmentID)
                );
              })
              .catch(function (err) {
                $(".fullScreenSpin").css("display", "none");
  ...
  datatablerecords: () => {
    return Template.instance()
      ...
      .sort(function (a, b) {
        ...
        return a.appointmentdate.toUpperCase() > b.appointmentdate.toUpperCase()
          ? 1
          : -1;
      });
  },
