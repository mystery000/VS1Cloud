                  ...
                  .DataTable({
                    ...
                    fnInitComplete: function () {
                      ...
                      if (data.Params.Search.replace(/\s/g, "") == "") {
                        $(
                          "<button class='btn btn-danger btnHideDeleted' type='button' id='btnHideDeleted' style='padding: 4px 10px; font-size: 16px; margin-left: 8px !important;'><i class='far fa-check-circle' style='margin-right: 5px'></i>Hide Deleted</button>"
                        ).insertAfter("#tblInvoicelist_filter");
                      } else {
                        $(
                          "<button class='btn btn-primary btnViewDeleted' type='button' id='btnViewDeleted' style='padding: 4px 10px; font-size: 16px; margin-left: 8px !important;'><i class='fa fa-trash' style='margin-right: 5px'></i>View Deleted</button>"
                        ).insertAfter("#tblInvoicelist_filter");
                      }
                      $(
...
Template.invoicelist.events({
  ...
  "click .resetTable": function (event) {
    ...
    let reset_data = [
      ...
      { label: "Comments", class: "colComments", active: false },
      {
        label: custFields[0].custfieldlabel,
        class: "colSaleCustField1",
        active: custFields[0].active,
      },
      {
        label: custFields[1].custfieldlabel,
        class: "colSaleCustField2",
        active: custFields[1].active,
      },
      {
        label: custFields[2].custfieldlabel,
        class: "colSaleCustField3",
        active: custFields[2].active,
      },
    ];
  ...
  "click .btnRefresh": async function () {
    ...
    sideBarService
      ...
      .then(async function (dataCustPay) {
        addVS1Data("TCustomerPaymentList", JSON.stringify(dataCustPay))
          .then(function (datareturn) {})
          .catch(function (err) {});
      })
      .catch(function (err) {});
    ...
    sideBarService
      ...
      .then(async function (dataInvoice) {
        addVS1Data("TInvoiceList", JSON.stringify(dataInvoice))
          .then(function (datareturn) {
            sideBarService
              .getTPaymentList(
                prevMonth11Date,
              ...
              .then(async function (dataPaymentList) {
                await addVS1Data(
                  "TPaymentList",
                  JSON.stringify(dataPaymentList)
                )
                  .then(function (datareturn) {
                    setTimeout(function () {
