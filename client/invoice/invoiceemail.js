...
Template.invoiceemail.onCreated(function () {
  ...
  let headerStructure = [
    { index: 0, label: '<div class="custom-control custom-checkbox chkBox" style="width:15px;"><input class="custom-control-input chkBoxAll" type="checkbox" id="formCheck-All"><label class="custom-control-label chkBoxAll" for="formCheck-All"></label></div>', class:'colChkBox', active: true, display: true, width: "20" },
...
Template.invoiceemail.onRendered(function () {
  ...
  $('#tblInvoicelistemail tbody').on('click', 'td:not(.colChkBox)', function () {
    var listData = $(this).closest('tr').children('td.colSalesNo').text();
    if (listData) {
      FlowRouter.go('/invoicecard?id=' + listData);
    }
  });
