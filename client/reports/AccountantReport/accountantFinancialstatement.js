...
import './accountantFinancialstatement.html';
...
Template.accountant_financialstatement.onCreated(() => {
    ...
    templateObject.retainedEarnings = new ReactiveVar();
    templateObject.retainedEarnings.set(0);
    templateObject.netIncome = new ReactiveVar();
    templateObject.netIncome.set(0);
});

Template.accountant_financialstatement.onRendered(() => {
    ...
    templateObject.getStartBalanceSheet = async(dateAsOf) => {
        LoadingOverlay.show();

        let data = await reportService.getBalanceSheetReport(dateAsOf);
        if (data.balancesheetreport) {
            for (let i = 0, len = data.balancesheetreport.length; i < len; i++) {
                if (data.balancesheetreport[i]["ACCNAME"] == "Retained Earnings") {
                    templateObject.retainedEarnings.set(data.balancesheetreport[i]["Header Account Total"]);
                }
            }
        }
    ...
    // var endDate = new Date(new Date().getFullYear(), (parseInt(endMonth)), 0);
    var startDate = new Date(new Date().getFullYear() - 1, (parseInt(endMonth)), 1);
    // templateObject.endDate.set(endDate.getDate() + " " + months[parseInt(endMonth) - 1] + " " + new Date().getFullYear());
    // var getLoadDate = moment(endDate).format("YYYY-MM-DD");
    var getStartDate = moment(startDate).format("YYYY-MM-DD");
    // templateObject.getBalanceSheetReports(getLoadDate);

    templateObject.getStartBalanceSheet(getStartDate);
            ...
            try {
                ...
                if (data.profitandlossreport) {
                    ...
                    for (let i = 0; i < accountData.length; i++) {
                        ...
                        if (accountType == "Net Income") {
                            templateObject.netIncome.set(accountData[i]["TotalAmountEx"]);
                        }
...
Template.accountant_financialstatement.events({
    ...
    "click .btnExportReport": function() {
        ...
        var opt = {
            margin: 0.8,
            filename: 'accountant-financialstatement.pdf',
            image: {
                type: 'jpeg',
                quality: 0.98
            },
    ...
    "click #btnSaveEditor": function(event) {
        ...
        if ($("#editorType").val() == "title") {
            ...
            $("#page-8-content-prt").html(elmnt + $("#page-9-content").html());
        } else {
...
Template.accountant_financialstatement.helpers({
    ...
    retainedEarnings: () => {
        return utilityService.modifynegativeCurrencyFormat(Template.instance().retainedEarnings.get());
    },

    netIncome: () => {
        return utilityService.modifynegativeCurrencyFormat(Template.instance().netIncome.get());
    },

    totalRetainedEarnings: () => {
        return utilityService.modifynegativeCurrencyFormat(parseFloat(Template.instance().retainedEarnings.get()) + parseFloat(Template.instance().netIncome.get()));
    },
});