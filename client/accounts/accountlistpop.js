...
Template.accountlistpop.onCreated(() => {
    ...
    templateObject.tableheaderrecords1.set(headerStructure);
});
...
Template.accountlistpop.onRendered(function() {
    ...
    $( "#accountListModal1" ).on('shown.bs.modal', function(){
        setTimeout(function() {
            $('#tblAccountListPop_filter .form-control-sm').get(0).focus()
        }, 500);
    });
...
Template.accountlistpop.helpers({
    ...
    purchaseCloudPreferenceRec: () => {
        return CloudPreference.findOne({ userid: localStorage.getItem('mycloudLogonID'), PrefName: 'creditcard' });
    },
    purchaseCloudGridPreferenceRec: () => {
        return CloudPreference.findOne({ userid: localStorage.getItem('mycloudLogonID'), PrefName: 'tblCreditLine' });
    },
    salesCloudPreferenceRec: () => {
        return CloudPreference.findOne({
            userid: localStorage.getItem('mycloudLogonID'),
            PrefName: 'tblAccountListPop'
        });
    },
    uploadedFiles: () => {
