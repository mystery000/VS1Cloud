...
Template.datatablelist.onRendered(async function () {
  ...
  function MakeNegative() {
    ...
    $("td.colStatus").each(function () {
      ...
      if ($(this).text() == "Not Converted")
        $(this).addClass("text-NotConverted");
  ...
  let indexDBLowercase =
    templateObject.data.lowercaseDataName || indexDBName.toLowerCase();
      ...
      try {
        getVS1Data("VS1_Customize").then(function (dataObject) {
          if (dataObject.length == 0) {
            ...
            if (
              data.ProcessLog.Obj != undefined &&
              data.ProcessLog.Obj.CustomLayout.length > 0
            ) {
              for (
                let i = 0;
  ...
  templateObject.showCustomFieldDisplaySettings = async function (
    savedHeaderInfo
  ) {
    let custFields = [];
    let customData = {};
        ...
        getVS1Data(indexDBName)
          .then(function (dataObject) {
            if (dataObject.length == 0) {
              ...
              templateObject.data.apiName
                .apply(that, params)
                .then(function (dataReturn) {
                  addVS1Data(indexDBName, JSON.stringify(dataReturn)).then(
                    function () {
                      resolve(dataReturn);
                    }
                  );
    ...
    if (data != [] && data.length != 0) {
      ...
      if (isEx == false) {
        ...
        for (let i = 0; i < data[lowercaseData].length; i++) {
          let dataList = templateObject.data.exdatahandler(
            data[lowercaseData][i]
          );
          if (dataList.length != 0) {
            if (templateObject.data.isMultipleRows) {
              dataList.map((item) => {
    ...
    const tabledraw = () => {
      $("#" + currenttablename)
        .DataTable({
          ...
          info: true,
          responsive: true,
          order: templateObject.data.orderby
            ? eval(templateObject.data.orderby)
            : [[1, "asc"]],
          // "autoWidth": false,
          action: function () {
            $("#" + currenttablename)
          ...
          fnDrawCallback: function (oSettings) {
            ...
            $(
              ...
              let customerSearch = $(
                "#" + currenttablename + "_filter input"
              ).val();
              ...
              templateObject.data.apiName
                ...
                .then(function (dataObjectnew) {
                  for (
                    let j = 0;
                    j < dataObjectnew[indexDBLowercase].length;
                    j++
                  ) {
                    var dataList = templateObject.data.datahandler(
                      dataObjectnew[indexDBLowercase][j]
                    );
                    splashDataArray.push(dataList);
                  }
          ...
          fnInitComplete: function (oSettings) {
            if (data.Params) {
              ...
              if (templateObject.data.viewConvertedButton == true) {
                ...
                $(
                  "<button class='btn btn-danger btnHideConverted' type='button' id='btnHideConverted' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;background-color: #f6c23e !important;border-color: #f6c23e!important;'><i class='far fa-check-circle' style='margin-right: 5px'></i>Hide Converted</button>"
                ).insertAfter("#" + currenttablename + "_filter");
                $(
                  "<button class='btn btn-primary btnViewConverted' type='button' id='btnViewConverted' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;background-color: #1cc88a !important;border-color: #1cc88a!important;'><i class='fa fa-trash' style='margin-right: 5px'></i>View Converted</button>"
                ).insertAfter("#" + currenttablename + "_filter");
                // }else{
              ...
              if (templateObject.data.showPlusButtonCRM == true) {
                ...
                $(
                  "<button class='btn btn-primary " +
                    ...
                    showPlusButtonDataTarget +
                    "' type='button' style='padding: 4px 10px; font-size: 16px; margin-left: 12px !important;'><i class='fas fa-plus'></i></button>"
                ).insertAfter("#" + currenttablename + "_filter");
              }

              if (data.Params.Search.replace(/\s/g, "") == "") {
                $(
                  "<button class='btn btn-danger btnHideDeleted' type='button' id='btnHideDeleted' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;'><i class='far fa-check-circle' style='margin-right: 5px'></i>" +
                    hideViewDeletedLabel +
                    "</button>"
                ).insertAfter("#" + currenttablename + "_filter");
              } else {
                if (
                  data.Params.Search == "IsBill = true and IsCheque != true"
                ) {
                  $(
                    "<button class='btn btn-danger btnHideDeleted' type='button' id='btnHideDeleted' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;'><i class='far fa-check-circle' style='margin-right: 5px'></i>" +
                      hideViewDeletedLabel +
                  ...
                  $(
                    "<button class='btn btn-primary btnViewDeleted' type='button' id='btnViewDeleted' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;'><i class='fa fa-trash' style='margin-right: 5px'></i>" +
                      activeViewDeletedLabel +
                      "</button>"
                  ).insertAfter("#" + currenttablename + "_filter");
                }
              }
            ...
            $(
              "<button class='btn btn-primary btnRefreshTable' type='button' id='btnRefreshTable' style='padding: 4px 10px; font-size: 16px; margin-left: 14px !important;'><i class='fas fa-search-plus' style='margin-right: 5px'></i>Search</button>"
            ).insertAfter("#" + currenttablename + "_filter");
          },
          fnInfoCallback: function (
            ...
            return (
              "Showing " + iStart + " to " + iEnd + " of " + countTableData
            );
          },
        })
        .on("page", function () {
      ...
      setTimeout(async function () {
        await $("div.dataTables_filter input").addClass(
    ...
    function getColDef() {
      ...
      for (let i = 0; i < $(".displaySettings").length; i++) {
        ...
        for (let i = 0; i < items.length; i++) {
          let tLabel =
            items[i]?.label?.indexOf("#") >= 0
              ? items[i].label.substr(1)
              : items[i].label;
          let rLabel =
            lineItemObj?.label?.indexOf("#") >= 0
              ? lineItemObj.label.substr(1)
              : lineItemObj.label;
          if (tLabel == rLabel) {
      ...
      if (items.length > 0) {
        for (let i = 0; i < items.length; i++) {
          let item = {
            targets: i,
            className:
              items[i]?.label?.includes("#") == false
                ? items[i].class
                : items[i].class + " hiddenColumn",
...
Template.datatablelist.events({
  ...
  "click .thisweek": function () {
    ...
    let thisWeekFirstDay = new Date(
      utc - currentBeginDate.getDay() * 1000 * 3600 * 24
    );
  ...
  "click .previousweek": function () {
    ...
    let previousWeekFirstDay = new Date(
      utc - (currentBeginDate.getDay() + 7) * 1000 * 3600 * 24
    );
    let previousWeekLastDay = new Date(
      utc - (currentBeginDate.getDay() + 1) * 1000 * 3600 * 24
    );
    ...
    var toDateERPFrom =
      previousWeekFirstDay.getFullYear() +
      "-" +
      previousWeekFromMonth +
      "-" +
      previousWeekFromDay;
    var toDateERPTo =
      previousWeekLastDay.getFullYear() +
      "-" +
      previousWeekToMonth +
      "-" +
      previousWeekToDate;
  ...
  "click .btnRefreshTable": async function (event) {
    ...
    if (dataSearchName.replace(/\s/g, "") != "") {
      ...
      templateObject.data.searchAPI
        .apply(that, paramArray)
        .then(function (data) {
          $(".btnRefreshTable").removeClass("btnSearchAlert");
  ...
  "click .resetTable": function (event) {
    ...
    oPost.open(
      ...
      URLRequest +
        ...
        erpGet.ERPPort +
        "/" +
        'erpapi/VS1_Cloud_Task/Method?Name="VS1_Customize"',
      true
    );
      ...
      if (oPost.readyState == 4 && oPost.status == 200) {
        ...
        if (myArrResponse.ProcessLog.Error) {
          ...
          sideBarService
            ...
            .then(async function (dataCustomize) {
              await addVS1Data("VS1_Customize", JSON.stringify(dataCustomize));
              templateObject.init_reset_data();
              templateObject.initCustomFieldDisplaySettings(
                "",
                currenttranstablename
              );
        ...
        if (segError[1] == ' "Unable to lock object') {
          swal(
            "WARNING",
            oPost.getResponseHeader("errormessage") + "Please try again!",
            "error"
          );
        } else {
          swal(
  ...
  "click .saveTable": async function (event) {
    ...
    try {
      ...
      let tableName = templateObject.data.tablename;
      let employeeId =
        parseInt(localStorage.getItem("mySessionEmployeeLoggedID")) || 0;
