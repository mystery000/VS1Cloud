...
Template.creditcard.onRendered(() => {
  ...
  templateObject.hasFollowings = async function () {
    ...
    if (getso_id[1]) {
      ...
      var isRepeated = creditData.fields.RepeatedFrom;
  ...
  function showCreditData1(template_title, number, bprint) {
    ...
    $("#tblCreditLine > tbody > tr").each(function () {
      ...
      let accountName = $("#" + lineID + " .es-input").val();
      let colMemo = $("#" + lineID + " .colMemo").text();
      let colTaxAmount = $("#" + lineID + " .colTaxAmount")
        .first()
        .text();

      let colAmount = $("#" + lineID + " .colAmount")
        .first()
        .val();
    ...
    for (let i in data.tsuppliervs1) {
      if (data.tsuppliervs1.hasOwnProperty(i)) {
        let supplierrecordObj = {
          ...
          statecode:
            data.tsuppliervs1[i].State + " " + data.tsuppliervs1[i].Postcode ||
            " ",
    ...
    getVS1Data("TSupplierVS1")
      .then(function (dataObject) {
        if (dataObject.length === 0) {
          ...
          if (!added) {
            contactService
              .getOneSupplierDataEx(supplierID)
              .then(function (data) {
                setSupplierByID(data);
              });
          }
      ...
      templateObject.getCreditData = function () {
        getVS1Data("TCredit")
          .then(function (dataObject) {
            if (dataObject.length == 0) {
              purchaseService
                .getOneCreditData(currentCredit)
                .then(function (data) {
                  $(".fullScreenSpin").css("display", "none");
                  let lineItems = [];
                  let lineItemObj = {};
                  ...
                  FxGlobalFunctions.handleChangedCurrency(
                    data.fields.ForeignExchangeCode,
                    defaultCurrencyCode
                  );
              ...
              for (let d = 0; d < useData.length; d++) {
                if (parseInt(useData[d].fields.ID) === currentCredit) {
                  ...
                  let creditrecord = {
                    ...
                    department:
                      useData[d].fields.Lines[0].fields.LineClassName ||
                      defaultDept,
                  };
                  ...
                  FxGlobalFunctions.handleChangedCurrency(
                    useData[d].fields.ForeignExchangeCode,
                    defaultCurrencyCode
                  );
    ...
    if (selectLineID) {
      ...
      if ($(".printID").val() == "") {
        $("#" + selectLineID + " #lineAmount").text(
          $("#" + selectLineID + " .colAmountExChange").val()
        );
                  ...
                  .then(function (data) {
                    ...
                    setTimeout(function () {
                      let supplierRecord = {
                        ...
                        routingNumber:
                          data.tsuppliervs1[0].fields.RoutingNumber || "",
                        bankAccountName:
                          data.tsuppliervs1[0].fields.BankAccountName || "",
                        bankAccountBSB:
                          data.tsuppliervs1[0].fields.BankAccountBSB || "",
                        bankAccountNo:
                          data.tsuppliervs1[0].fields.BankAccountNo || "",
                        foreignExchangeCode:
                          data.tsuppliervs1[0].fields.ForeignExchangeCode ||
                          CountryAbbr,
                  ...
                  if (
                    ...
                    setTimeout(function () {
                      let supplierRecord = {
                        ...
                        routingNumber:
                          data.tsuppliervs1[i].fields.RoutingNumber || "",
                        bankAccountName:
                          data.tsuppliervs1[i].fields.BankAccountName || "",
                        bankAccountBSB:
                          data.tsuppliervs1[i].fields.BankAccountBSB || "",
                        bankAccountNo:
                          data.tsuppliervs1[i].fields.BankAccountNo || "",
                        foreignExchangeCode:
                          data.tsuppliervs1[i].fields.ForeignExchangeCode ||
                          CountryAbbr,
          ...
          accountService.getAccountListVS1().then(function (data) {
            ...
            for (let i = 0; i < data.taccountvs1.length; i++) {
              var dataList = [
                ...
                utilityService.modifynegativeCurrencyFormat(
                  Math.floor(data.taccountvs1[i].Balance * 100) / 100
                ),
...
Template.creditcard.helpers({
  ...
  displayFieldColspan: (displayfield) => {
    if (
      ["Amount (Ex)", "Amount (Inc)", "Tax Amt"].includes(
...
Template.creditcard.events({
  ...
  "blur .colAmountExChange": function (event) {
    ...
    if ($(".printID").val() == "") {
      $("#" + targetID + " #lineAmount").text(
        $("#" + targetID + " .colAmountExChange").val()
      );
  ...
  "click .lineTaxAmount": function (event) {
    ...
    $("#tblCreditLine tbody tr .lineTaxAmount").attr(
      "data-target",
      "#taxDetailModal"
    );
  },
  ...
  "click .printConfirm": async function (event) {
    ...
    setTimeout(async function () {
      ...
      if (
        ...
        $("#print_credit_second").is(":checked")
      ) {
      ...
      let creditID = FlowRouter.current().queryParams.id
      ...
      values.forEach((value) => {
        ...
        temp.HostURL = $(location).attr("protocal")
        ...
        if (temp.BasedOnType.includes("P")) {
          if (temp.FormID == 1) {
            ...
            if (temp.FormID == 21) Meteor.call("sendNormalEmail", temp);
          }
        }
      });
    }, delayTimeAfterSound);
    ...
    setTimeout(function () {
      ...
      if (suppliername.val() === "") {
        ...
        $("#tblCreditLine > tbody > tr").each(function () {
          ...
          let tdtaxCode =
            $("#" + lineID + " .lineTaxCode").val() || loggedTaxCodePurchaseInc;
                    ...
                    function (error, result) {
                      if (error && error.error === "error") {
                        ...
                        swal({
                          title: "SUCCESS",
                          text:
                            "Email Sent To Supplier: " + checkEmailData + " ",
                          type: "success",
                          showCancelButton: false,
                          confirmButtonText: "OK",
                        }).then((result) => {
