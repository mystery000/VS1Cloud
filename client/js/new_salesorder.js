...
var template_list = ["Sales Order", "Delivery Docket"];
var noHasTotals = [
  "Customer Payment",
  "Customer Statement",
  "Supplier Payment",
...
Template.new_salesorder.onRendered(function () {
  ...
  function showSealsOrder1(template_title, number, bprint) {
    ...
    $("#tblSalesOrderLine > tbody > tr").each(function () {
      ...
      let taxamount = $("#" + lineID + " .lineTaxAmount")
        .first()
        .text();
      let tdtaxrate = $("#" + lineID + " .lineTaxRate").text();
      let tdtaxCode =
        $("#" + lineID + " .lineTaxCode").val() || loggedTaxCodeSalesInc;
      let tdlineamt = $("#" + lineID + " .colAmountEx")
        .first()
        .text();
  ...
  if (url.includes("id") && url.includes("total")) {
    ...
    if (getso_id != "") {
      let salesOrderID = parseInt(getso_id);
      $(".printID").attr("id", salesOrderID);
      templateObject.getSalesOrderData = function () {
        getVS1Data("TSalesOrderEx")
          .then(function (dataObject) {
            if (dataObject.length === 0) {
              accountService
                .getOneSalesOrderdataEx(salesOrderID)
                .then(function (data) {
                  let lineItems = [];
                  let lineItemObj = {};
                  let lineItemsTable = [];
                  ...
                  if (data.fields.Lines != null) {
                    if (data.fields.Lines.length) {
                      ...
                      let AmountGbp = data.fields.Lines.fields.TotalLineAmountInc.toLocaleString(
                        undefined,
                        {
                          minimumFractionDigits: 2,
                        }
                    ...
                    if (clientList) {
                      for (var i = 0; i < clientList.length; i++) {
                        if (
                          ...
                          salesorderrecord.firstname =
                            clientList[i].firstname || "";
                          salesorderrecord.lastname =
              ...
              for (let d = 0; d < useData.length; d++) {
                if (parseInt(useData[d].fields.ID) === salesOrderID) {
                  ...
                  let totalDiscount = utilityService
                    .modifynegativeCurrencyFormat(
                      useData[d].fields.TotalDiscount
                    )
                    .toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    });
                  ...
                  let totalBalance =
                    currencySymbol +
                    "" +
                    useData[d].fields.TotalBalance.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    });
                  let totalPaidAmount =
                    currencySymbol +
                  ...
                  $("#exchange_rate").val(
                    useData[d].fields.ForeignExchangeRate
                  );
              ...
              if (!added) {
                try {
                  accountService
                    .getOneQuotedataEx(salesOrderID)
                    .then(function (data) {
                      let lineItems = [];
                      let lineItemObj = {};
                      let lineItemsTable = [];
          ...
          .catch(function (err) {
            accountService
              .getOneSalesOrderdataEx(currentQuote)
              .then(function (data) {
                LoadingOverlay.hide();
                let lineItems = [];
                let lineItemObj = {};
      ...
      async function addAttachment() {
        ...
        reader.onloadend = function () {
          ...
          var htmlmailBody =
            ...
            "                        Please find payment for Sales Order <span> " +
            erpInvoiceId +
            "</span> attached below." +
            "                    </td>" +
            "                </tr>" +
    ...
    if (getso_id[1]) {
      ...
      templateObject.getSalesOrderData = function () {
        accountService
          .getOneQuotedataEx(currentSalesOrder)
          .then(function (data) {
            LoadingOverlay.hide();
            let lineItems = [];
            let lineItemObj = {};
            ...
            if (data.fields.Lines != null) {
              if (data.fields.Lines.length) {
                ...
                let TaxRateGbp =
                  currencySymbol + "" + data.fields.Lines.fields.LineTaxRate;
                lineItemObj = {
                  lineID: Random.id(),
                  id: data.fields.Lines.fields.ID || "",
                  description:
      ...
      templateObject.getSalesOrderData = function () {
        getVS1Data("TSalesOrderEx")
          .then(function (dataObject) {
            if (dataObject.length == 0) {
              accountService
                .getOneSalesOrderdataEx(currentSalesOrder)
                .then(function (data) {
                  LoadingOverlay.hide();
                  let lineItems = [];
                  let lineItemObj = {};
      ...
      $tblrows.each(function (index) {
        ...
        if (!isNaN(discountTotal)) {
          ...
          document.getElementById(
            "subtotal_discount"
          ).innerHTML = utilityService.modifynegativeCurrencyFormat(
            subDiscountTotal
          );
        }
        $tblrow
        ...
        if (!isNaN(subGrandTotal) && !isNaN(taxGrandTotal)) {
          ...
          document.getElementById(
            "grandTotal"
          ).innerHTML = utilityService.modifynegativeCurrencyFormat(GrandTotal);
          document.getElementById(
            "balanceDue"
          ).innerHTML = utilityService.modifynegativeCurrencyFormat(GrandTotal);
          document.getElementById(
  ...
  exportSalesToPdf = async function (template_title, number) {
    if (template_title == "Sales Order") {
      await showSealsOrder1(template_title, number, true);
    }
        ...
        if (dataObject.length == 0) {
          productService.getNewProductListVS1().then(function (data) {
            ...
            for (let i = 0; i < data.tproductvs1.length; i++) {
              var dataList = [
                ...
                utilityService.modifynegativeCurrencyFormat(
                  Math.floor(data.tproductvs1[i].BuyQty1Cost * 100) / 100
                ),
                utilityService.modifynegativeCurrencyFormat(
                  Math.floor(data.tproductvs1[i].SellQty1Price * 100) / 100
                ),
            ...
            localStorage.setItem(
              "VS1SalesProductList",
              JSON.stringify(splashArrayProductList)
            );
          ...
          for (let i = 0; i < useData.length; i++) {
            var dataList = [
              ...
              utilityService.modifynegativeCurrencyFormat(
                Math.floor(useData[i].fields.BuyQty1Cost * 100) / 100
              ),
              utilityService.modifynegativeCurrencyFormat(
                Math.floor(useData[i].fields.SellQty1Price * 100) / 100
              ),
            ...
            if (useData[i].fields.ExtraSellPrice != null) {
              for (
                ...
                let lineExtaSellObj = {
                  clienttype:
                    useData[i].fields.ExtraSellPrice[e].fields.ClientTypeName ||
                    "",
                  productname:
                    useData[i].fields.ExtraSellPrice[e].fields.ProductName ||
                    useData[i].fields.ProductName,
                  price:
                    utilityService.modifynegativeCurrencyFormat(
...
Template.new_salesorder.helpers({
  ...
  printEmailData: async () => {
    ...
    $("#tblSalesOrderLine > tbody > tr").each(function () {
      ...
      let tdExpiryDates = $("#" + lineID + " .colSerialNo").attr(
        "data-expirydates"
      );
    ...
    for (let l = 0; l < lineItemsForm.length; l++) {
      stringQuery =
        ...
        l +
        "=" +
        lineItemsForm[l].fields.ProductName +
        "&price" +
        l +
        "=" +
        lineItemsForm[l].fields.LinePrice +
        "&qty" +
        l +
        "=" +
        lineItemsForm[l].fields.UOMQtySold +
        "&";
    }
    stringQuery =
      stringQuery +
    ...
    var htmlmailBody =
      ...
      '                                            <span class="doc-details" style="color: #999999; font-size: 12px; text-align: center; margin: 0 auto; text-transform: uppercase;">Sales Order No. ' +
      erpInvoiceId +
      " Details</span>" +
      "                                        </td>" +
      "                                    </tr>" +
      ...
      "                                                                <tbody>" +
      "                                                                    <tr>" +
      '                                                                        <td> <a href="https://www.depot.vs1cloud.com/stripe/' +
      stringQuery +
      ...
      '                                    <a href="mailto:' +
      mailFrom +
      '" style="color: #999999; font-size: 12px; text-align: center;">Contact Us</a>' +
...
Template.new_salesorder.events({
  ...
  "click .lineTaxRate": function (event) {
    ...
    $("#tblSalesOrderLine tbody tr .lineTaxRate").attr(
      "data-target",
      "#taxRateListModal"
    );
    var targetID = $(event.target).closest("tr").attr("id"); // table row ID
  ...
  "click .lineTaxAmount": function (event) {
    ...
    $("#tblSalesOrderLine tbody tr .lineTaxAmount").attr(
      "data-toggle",
      "modal"
    );
    $("#tblSalesOrderLine tbody tr .lineTaxAmount").attr(
  ...
  "click .printConfirm": async function (event) {
    ...
    setTimeout(async function () {
      ...
      sideBarService
        .getTemplateNameandEmployeId("Sales Orders", emid, 1)
        .then(function (data) {
      ...
      sideBarService
        .getTemplateNameandEmployeId("Sales Orders", emid, 2)
        .then(function (data) {
          templateid = data.ttemplatesettings;
          var id = templateid[0].fields.ID;
          objDetails = {
      ...
      sideBarService
        .getTemplateNameandEmployeId("Sales Orders", emid, 3)
        .then(function (data) {
          templateid = data.ttemplatesettings;
          var id = templateid[0].fields.ID;
          objDetails = {
      ...
      if (
        $("#print_sales_order").is(":checked") ||
        $("#print_sales_order_second").is(":checked")
      ) {
  ...
  "click .btnDeleteFollowingSOs": async function (event) {
    ...
    setTimeout(async function () {
      swal({
        ...
        text:
          "Do you wish to delete this transaction and all others associated with it moving forward?",
        type: "question",
        ...
        if (result.value) {
          ...
          if (getso_id[1]) {
            currentInvoice = parseInt(currentInvoice);
            var soData = await salesService.getOneSalesOrderdataEx(
              currentInvoice
            );
  ...
  "click .btnSave": (event, templateObject) => {
    playSaveAudio();
    // let templateObject1 = Template.instance();
    let salesService = new SalesBoardService();
    ...
    setTimeout(function () {
      ...
      if (customername.val() === "") {
        ...
        salesService
          ...
          .then(function (data) {
            sideBarService
              .getAllSalesOrderList(initialDataLoad, 0)
              .then(function (dataUpdated) {
                addVS1Data("TSalesOrderEx", JSON.stringify(dataUpdated))
                  .then(function (dataReturn) {
                    saveFunc();
                  })
                  .catch((error) => {
  ...
  "click .btnSaveGridSettings": async function (event) {
    ...
    setTimeout(async function () {
      ...
      try {
        ...
        let employeeId =
          parseInt(Session.get("mySessionEmployeeLoggedID")) || 0;
        let added = await sideBarService.saveNewCustomFields(
          erpGet,
          tableName,
          employeeId,
          lineItems
  ...
  "click .btnResetGridSettings": function (event) {
    ...
    $(".displaySettings").each(function (index) {
      ...
      if (
        reset_data[index].class === "AmountEx" ||
        reset_data[index].class === "UnitPriceEx"
      ) {
        $(title).html(
          reset_data[index].label + `<i class="fas fa-random fa-trans"></i>`
        );
      } else if (
        reset_data[index].class === "AmountInc" ||
        reset_data[index].class === "UnitPriceInc"
      ) {
        $(title).html(
          reset_data[index].label + `<i class="fas fa-random"></i>`
        );
      } else {
  ...
  "click .payNow": function () {
    ...
    if (stripe_id != "") {
      ...
      swal({
        title: "WARNING",
        text:
          "Don't have a Stripe account yet, Please click Ok to set up Stripe.",
        type: "warning",
        showCancelButton: false,
        confirmButtonText: "OK",
      }).then((result) => {
  ...
  "click #btnCopyToInvoice": async function () {
    ...
    setTimeout(async function () {
      ...
      if (getso_id[1]) {
        ...
        var convertedStartDate = subStartDate
          ? subStartDate.split("-")[2] +
            "/" +
            subStartDate.split("-")[1] +
            "/" +
            subStartDate.split("-")[0]
          : "";
        var convertedFinishDate = subFinishDate
          ? subFinishDate.split("-")[2] +
            "/" +
            subFinishDate.split("-")[1] +
            "/" +
            subFinishDate.split("-")[0]
          : "";
        ...
        if (radioFrequency == "frequencyMonthly") {
          ...
          document.getElementById("oneTimeOnlySettings").style.display =
            "block";
          document.getElementById("monthlySettings").style.display = "none";
          document.getElementById("weeklySettings").style.display = "none";
          document.getElementById("dailySettings").style.display = "none";
    ...
    setTimeout(async function () {
      ...
      if (values.includes("F")) {
        if (radioFrequency == "frequencyMonthly") {
          ...
          dailyRadioOption = $("#dailySettings input[type=radio]:checked").attr(
            "id"
          );
      ...
      convertedStartDate = startDate
        ? startDate.split("/")[2] +
          ...
          startDate.split("/")[0]
        : "";
      convertedFinishDate = finishDate
        ? finishDate.split("/")[2] +
          "-" +
          finishDate.split("/")[1] +
          "-" +
          finishDate.split("/")[0]
        : "";
      sDate = convertedStartDate
        ? moment(convertedStartDate + " " + copyStartTime).format(
            "YYYY-MM-DD HH:mm"
          )
        : moment().format("YYYY-MM-DD HH:mm");
      fDate = convertedFinishDate
        ? moment(convertedFinishDate + " " + copyStartTime).format(
            "YYYY-MM-DD HH:mm"
          )
        : moment().format("YYYY-MM-DD HH:mm");
      sDate2 = convertedStartDate
        ? moment(convertedStartDate).format("YYYY-MM-DD")
        : moment().format("YYYY-MM-DD");
      fDate2 = convertedFinishDate
        ? moment(convertedFinishDate).format("YYYY-MM-DD")
        : moment().format("YYYY-MM-DD");
      $(".fullScreenSpin").css("display", "inline-block");
      var url = FlowRouter.current().path;
      if (
        ...
        if (getso_id[1]) {
          ...
          let repeatDates = [];
          if (
            radioFrequency == "frequencyDaily" ||
            radioFrequency == "frequencyOnetimeonly"
          ) {
            period = "Daily"; // 0
          ...
          if (radioFrequency == "frequencyWeekly") {
            ...
            for (i = 0; i < arrSelectDays.length; i++) {
              ...
              if (parseInt(arrSelectDays[i]) == 0) weekdayObj.sunday = 1;
              if (parseInt(arrSelectDays[i]) == 1) weekdayObj.monday = 1;
              if (parseInt(arrSelectDays[i]) == 2) weekdayObj.tuesday = 1;
              if (parseInt(arrSelectDays[i]) == 3) weekdayObj.wednesday = 1;
              if (parseInt(arrSelectDays[i]) == 4) weekdayObj.thursday = 1;
              if (parseInt(arrSelectDays[i]) == 5) weekdayObj.friday = 1;
              if (parseInt(arrSelectDays[i]) == 6) weekdayObj.saturday = 1;
            }
          }
          if (radioFrequency == "frequencyMonthly") {
            period = "Monthly"; // 0
            repeatMonths = convertStrMonthToNum(ofMonths);
            repeatDates = getRepeatDates(
              sDate2,
              fDate2,
              repeatMonths,
              monthDate
          ...
          if (days.length > 0) {
            for (let x = 0; x < days.length; x++) {
              ...
              oPost.onreadystatechange = function () {
                if (oPost.readyState == 4 && oPost.status == 200) {
                  ...
                  var success = myArrResponse.ProcessLog.ResponseStatus.includes(
                    "OK"
                  );
            ...
            if (
              radioFrequency == "frequencyOnetimeonly" ||
              radioFrequency == "frequencyMonthly"
            ) {
              dayObj = {
                Name: "VS1_RepeatTrans",
                Params: {
  ...
  "click #btnMakeWorkOrder": async function (event) {
    ...
    if (!FlowRouter.current().queryParams.id) {
      swal({
        title: "Oooops...",
        text:
          "This sales order has not been saved yet, will save it first and then try again",
        type: "error",
        showCancelButton: false,
        confirmButtonText: "Ok",
      }).then((result) => {
      ...
      if (lines.length != orderlines.length) {
        ...
        swal({
          title: "Oooops...",
          text:
            "Changes for lines has not been saved yet, will save it first and then try again",
          type: "error",
          showCancelButton: true,
          confirmButtonText: "Ok",
        }).then((result) => {
        ...
        for (let j = 0; j < orderlines.length; j++) {
          if (
            lineProducts.indexOf(
              $(orderlines[j]).find(".lineProductName").val()
            ) == -1
          ) {
            changeAble = false;
            swal({
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (isBOMProduct == true) {
              ...
              workorderList.map((order) => {
                if (
                  order.SalesOrderID == salesOrderRecord.id &&
                  order.Line.fields.productName == lines[i].item
                ) {
                  isExisting = true;
                  isAvailable = false;
                }
  ...
  "change .exchange-rate-js, change #tblSalesOrderLine tbody input": (
    e,
    ui
  ) => {
    FxGlobalFunctions.convertToForeignEveryFieldsInTableId(
      "#tblSalesOrderLine"
    );
